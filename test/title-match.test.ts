...
describe("title-match", function () {
  ...
  describe("extractYear and yearGatePasses", function () {
    ...
    it("requires a known candidate year", function () {
      assert.isFalse(yearGatePasses("2024", undefined));
      assert.isFalse(yearGatePasses(undefined, undefined));
    });

    it("passes when the preprint year is unknown (timeline not disproven)", function () {
      assert.isTrue(yearGatePasses(undefined, "2025"));
      assert.isTrue(yearGatePasses("", "2025"));
    });
  });
